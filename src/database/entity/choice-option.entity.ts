...
@Entity({ name: 'choice_option', comment: '선택지' })
export class ChoiceOption {
	@PrimaryGeneratedColumn({ type: 'int' })
	public id: number;

	@Column({ name: 'page_id', type: 'int', comment: '선택지가 속한 페이지 id' })
	public pageId: number;

	@Column({ name: 'move_target_type', type: 'smallint', comment: '1: 페이지, 2: 엔딩' })
	public moveTargetType: MoveTargetType | null;

	@Column({ name: 'target_id', type: 'int', nullable: true, comment: '다음 페이지 id or 엔딩 id' })
	public targetId: number | null;

	@Column({ name: 'order_num', type: 'smallint', comment: '선택지 순서' })
